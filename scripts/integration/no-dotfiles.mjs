...
//
// Asserts: nothing crashes, every reported write lands inside the project, and
// neither the fake HOME nor the machine's real HOME gains a `~/.claude`.
//
 ...
 * (doctor findings), so the status comes back for the
...
// 3. doctor — read-only; must run cleanly with no ~/.claude at all.
console.log('\n── doctor ──')
...
// 4. The whole fix matrix. A bare `fix` never writes outside the repo.
console.log('\n── fix --yes (full matrix) ──')
...
assert(noUserClaude(), 'a bare fix --yes left ~/.claude alone')

// 5. Nothing leaked into the machine's real HOME.
assert(
